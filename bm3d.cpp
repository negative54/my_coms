...
,   const unsigned color_space
    ...
    //! Parameters
    const unsigned nHard = 39; //! Half size of the search window
    const unsigned nWien = 39; //! Half size of the search window
    ...
    const unsigned NHard = 8; //! Must be a power of 2
    const unsigned NWien = 8; //! Must be a power of 2
...
,   const float    tauMatch
    ...
    for(unsigned i=nHW;i<height-nHW;i=i+pHW)
    {
        int k=i*width+nHW;
        vector<pair<float,unsigned> > dist;

        for(unsigned j=nHW;j<width-nHW;j=j+pHW,k=k+pHW)
        {
            for (unsigned di = -nHW; di <= nHW; di++)
            {
                for (unsigned dj = 0; dj < Ns; dj++)
                {
                    const int dk = (int) (di * width + dj) - (int) nHW;

                    dist.push_back(make_pair((img[k]-img[k+dk])*(img[k]-img[k+dk]),k+dk));

                }
            }
            const unsigned nSx_r = (NHW > dist.size() ? closest_power_of_2(dist.size()) : NHW);
            partial_sort(dist.begin(), dist.begin() + nSx_r,dist.end(),ComparaisonFirst);
            for (unsigned n = 0; n < nSx_r; n++)
                patch_table[k].push_back(dist[n].second);
        }
